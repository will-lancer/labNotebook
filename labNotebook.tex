...
the whatever frequency setter thing. We fixed it, and switched the
...
We are back at it again. We are done with the experiment. Du encouraged
us to do more samples, so we are going to do that now.

We are going to do high-strength Neoprene (129N13).

\subsection{High-strength Neoprene: $T_2$}

We are going to do the $T_2$ measurement for high-strength Neoprene
now. Recall that the way to do this it to apply a $\pi/2$ pulse
and export the graph and fit it to $e^{-t/T_2}$. 

We took the picture but then I pressed a button that
messed up literally everything on the scope. So had
to call Du over and retake things.

Relevant values:
\begin{itemize}
    \item RF frequency matching: $21.01270 \pm 0.00001 \, \mathrm{MHz}$.
    \item $\pi/2$ pulse: $3.40 \pm 0.005 \, \mu \mathrm{s}$.
    \item Period: $650 \pm 0.5 \, \mathrm{ms}$.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/rawT2dataNeoprene.png}
    \caption{Raw $T_2$ data for high-strength Neoprene}
    \label{fig:raw_t_2_data_neoprene}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/fittedt2Neoprene.png}
    \caption{$T_2$ data for high-strength Neoprene, fit to $e^{-t/T_2}$ curve.}
    \label{fig:t_2_data_neoprene}
\end{figure}

We tried to measure $T_1$ for the high-strength Neoprene, but the
scope's trigger was super weird. We tried to fix it for a while,
and both of the TAs did as well. It didn't work. We tried to get
Du, but he was gone. So we just left.

\section{March 3, 2026}

WE BACK AT IT AGAIN!!! Du fixed the scope for us in about
two minutes, as always (he's the goat), and so now we're going
to measure $T_1$ and $T_2'$ for the high-strength Neoprene.

\subsection{High-strength Neoprene: $T_1$}

Relevant values:
\begin{itemize}
    \item RF frequency matching: $21.00766 \pm 0.00001 \, \mathrm{MHz}$.
    \item $\pi/2 = B$ pulse: $3.38 \pm 0.005 \, \mu \mathrm{s}$.
    \item $\pi = A$ pulse: $6.74 \pm 0.005 \, \mu \mathrm{s}$.
    \item Period: $650 \pm 0.5 \, \mathrm{ms}$.
\end{itemize}

Recall the procedure for $T_1$ measurement:
\begin{itemize}
    \item Find $A$ and $B$ pulses etc.
    \item Find total $\tau$ for it to return to its max value.
    \item Take $\sim 50$ data points, measuring the voltage at
    each time
    \item Fit the data to the $\sim (1 - e^{-\tau/T_1})$ curve;
    invert the data pre-zero to get a nice damped exponential fit.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/T1_neoprene_noinvert.png}
    \caption{$T_1$ curve with no inversion}
    \label{fig:raw_t_1_data_neoprene}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/T1_neprene_inverted.png}
    \caption{$T_1$ curve with inversion}
    \label{fig:t_1_data_neoprene}
\end{figure}
...
\begin{itemize}
    \item RF frequency matching: $21.00766 \pm 0.00001 \, \mathrm{MHz}$.
    \item $\pi/2 = A$ pulse: $6.74 \pm 0.005 \, \mu \mathrm{s}$.
    \item $\pi = B$ pulse: $3.38 \pm 0.005 \, \mu \mathrm{s}$.
    \item Period: $650 \pm 0.5 \, \mathrm{ms}$.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/t2prime_neoprene_rawdata.png}
    \caption{$T_2'$ neoprene raw data}
    \label{fig:t2_prime_neoprene_rawdata}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/t2prime_fitted_data.png}
    \caption{$T_2'$ neoprene fitted data}
    \label{fig:t2_prime_neoprene_fitted}
\end{figure}

\section{March 5, 2026}

We no longer have the Du by our side$\ldots$ please take
a moment of silence. We have Corliss, though, and he's great,
so we're chilling.

We're starting on X-ray diffraction and fluorescence today.
We had a discussion with Corliss to start the lab, and then
Kevin set us up with the equipment.

\subsection{X-ray diffraction}

Dis lab easy af!!!

We swept the following voltage values:
\begin{itemize}
    \item $35$ kV
    \item $34$ kV
    \item $\ldots$
...
We replaced the detector with the other detector.
Then adjusted the detector to be at $90^\circ$ to the
salt crystal, and measured the following distances between
the emitter-sample and sample-detector:
\begin{itemize}
    \item Emitter-sample: $5.5$ cm
    \item Sample-detector: $5.5$ cm
\end{itemize}
We then adjusted the salt crystal from before to be at $45^\circ$
to the detector.

We replaced the salt crystal with the FeZn sample.
We did the set-up detailed in the lab manual and collected
data for the FeZn sample. We will do the rest of fluorescence
next time.

\section{March 10, 2026}

I'm sick today, so Alo is doing all of the equipment-touching.
I'm just coaching her through it and doing the data analysis.

Alo set up the X-ray fluorescence set-up, adjusted things to be
$5.5 \unit{cm}$, and then started things up.

I had Chat 5.4 Extended Thinking fit the data to the set of linear
curves we want and make a table from the data as well. We got

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/diff_35-24kV.png}
    \caption{Linear fit lines for $35-24 \unit{kV}$ sweeps.}
    \label{fig:diff_35-24kV}
\end{figure}
...
\begin{table}[H]
    ...
    \hline
    \textbf{Voltage (kV)} & \textbf{Slope (Counts/deg)} & $\boldsymbol{\sigma_{\mathrm{Slope}}}$ & \textbf{y-intercept (counts)} & $\boldsymbol{\sigma_{\mathrm{intercept}}}$ \\
    \hline
    35 & 1196.95 & 25.66 & -4152.67 & 93.10 \\ \hline
    34 & 995.83  & 23.03 & -3579.30 & 85.65 \\ \hline
    33 & 1025.46 & 31.43 & -3822.32 & 124.35 \\ \hline
    32 & 913.26  & 24.57 & -3509.83 & 99.90 \\ \hline
    31 & 799.99  & 21.64 & -3172.94 & 89.61 \\ \hline
    30 & 699.66  & 18.42 & -2860.99 & 77.26 \\ \hline
    29 & 690.37  & 20.15 & -2937.61 & 89.51 \\ \hline
    28 & 582.29  & 15.26 & -2553.74 & 69.07 \\ \hline
    27 & 526.24  & 14.66 & -2406.54 & 69.39 \\ \hline
    26 & 478.72  & 14.58 & -2276.89 & 72.22 \\ \hline
    25 & 406.30  & 13.93 & -2007.01 & 72.06 \\ \hline
    24 & 389.65  & 12.63 & -2013.80 & 67.37 \\ \hline
    \end{tabular}
    \caption{Fitted Parameters and Uncertainties for Each Voltage}
    \label{tab:xrd_fit_parameters}
\end{table}

We took fluorescence data, but then realized that it was
around half the size that it should be (i.e. half the
number of counts), but its shape is qualitative what we'd
expect (four peaks; two sets of two peaks, one big, one small). 

We asked Kevin O'Shea why this was, and he said it may be because 
we are using the collimated emitter. We switched out our collimated 
emitter for the circular one, and re-ran our data on the FeZn sample.

It is much better. Its counts are very high, around 3x of
what they were before.

\section{March 12, 2026}

We retook FeZn. Fit its values to determine its peaks.
Used Chat to do so---I ran out of file uploads so Alo did it
for us.

Then we took the following samples:
\begin{itemize}
    \item Ti --- titanium
    \item Ag --- silver
    \item Rh --- rhodium
...
\section{March 24, 2026}

Long time no see---it was nice while it lasted.

Chat analyzed our data from last time and produced the following plots:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/feZn_rizz.png}
    \caption{Fluorescence calibration data for FeZn}
    \label{fig:fluor_fezn}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/moseley!!!.png}
    \caption{Fluorescence data for other samples}
    \label{fig:fluor_other_samples}
\end{figure}

\section{March 31, 2026}

Corliss told us to retake our fluor data for gold
and to take more samples to see if the trend of
the $L_\alpha$ and $L_\beta$ lines only appearing for
higher atomic numbers holds. So we measured the following
samples, measuring the reference FeZn again.
\begin{itemize}
    \item Gold
    \item Lead
    \item Uranium
...
These are ordered by atomic number as
...
See the analyzed data in the assets folder: it's the
only PDF in there.

\section{April 9th, 2026}

Starting Mossbauer today. Waited around an hour for Prof. Drees
to come help us get set up. Once he came over, we learned a bit
about the experiment and got the basic things set up. See the
pictures on your phone for the exact set-up.

We then proceeded to find PPE in the form of gloves that we got from
the laser labs downstairs. We then did the first part of the experiment,
i.e. the stuff on page 2 of the detailed instructions (as in, after the
page count resets at page 11---page 2 of that part).

Specific comment:
\begin{itemize}
    \item The left triangle should be around 227 and the right triangle around 350.
    You can see the csv files to recalibrate this on tuesday. We do not
    have enough time to use these today.
\end{itemize}

\section{April 14, 2026}

We did the calibration for the iron sample yesterday,
and today we are going to do some of the other experiments.
We are waiting for Drees to being the experiments, as we don't know
if we have to run them overnight or something.

We set the first experiment running and waited around thirty minutes to
verify the correct behavior. It looked good so we left.

\section{April 16, 2026}

Just started the data and left.

\section{April 21, 2026}

Did all of the data analysis today for our previous experiments. Didn't
start any new experiments because we didn't know how + Drees wasn't here.

We did the Fe57 analysis, the enriched iron analysis, and the isomer shift analysis
today.
